...
//            Handle error
            if (data.size() > 4) {
                ...
                for (int i = 4; i > 0; i--) {
                    dp[4 - i] = new DataPoint(myIdList.get(data.size() - i), myPltList.get(data.size() - i));
...
//                gridLabel.setLabelHorizontalHeight(50);
                ...
                for(int i=0;i<data.size();i++){
                    dp[i] = new DataPoint(myIdList.get(i), myPltList.get(i));
